import { z } from 'zod';
import { createStayRecord, getStayRecordById, getStayRecords, updateStayRecord, checkOutGuest, deleteStayRecord, getStayRecordStats, } from '../services/stayRecordService.js';
// Validation schemas
...
export async function createStayRecordController(req, res) {
    try {
        if (!req.user) {
...
export async function getStayRecordByIdController(req, res) {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
...
export async function getStayRecordsController(req, res) {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
        ...
        if (filters.booking_id)
            processedFilters.booking_id = filters.booking_id;
        if (filters.guest_id)
        ...
        if (typeof filters.has_incidents === 'boolean')
            processedFilters.has_incidents = filters.has_incidents;
...
// Update stay record
export async function updateStayRecordController(req, res) {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
        ...
        if (payload.check_in_time)
            updateData.check_in_time = new Date(payload.check_in_time);
        if (payload.check_out_time)
            updateData.check_out_time = new Date(payload.check_out_time);
        if (payload.notes !== undefined)
            updateData.notes = payload.notes;
        if (payload.room_condition !== undefined)
            updateData.room_condition = payload.room_condition;
        if (payload.amenities_used !== undefined)
            updateData.amenities_used = payload.amenities_used;
        if (payload.incidents !== undefined)
            updateData.incidents = payload.incidents;
...
// Check-out guest
export async function checkOutGuestController(req, res) {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
        ...
        const checkOutData = {};
        if (payload.check_out_time)
            checkOutData.check_out_time = new Date(payload.check_out_time);
        if (payload.notes !== undefined)
            checkOutData.notes = payload.notes;
        if (payload.room_condition !== undefined)
            checkOutData.room_condition = payload.room_condition;
        if (payload.incidents !== undefined)
            checkOutData.incidents = payload.incidents;
...
export async function deleteStayRecordController(req, res) {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
...
export async function getStayRecordStatsController(req, res) {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Unauthorized' });
        }
...
//# sourceMappingURL=stayRecordController.js.map