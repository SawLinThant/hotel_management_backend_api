...
async function assertRoomAvailable(room_id, from, to, excludeBookingId) {
    ...
    if (overlapping)
        throw Conflict('Room is not available for selected dates');
}
...
export async function listBookings(params) {
    ...
    if (params.check_in_from || params.check_in_to) {
        ...
        if (params.check_in_from)
            where.check_in_date.gte = params.check_in_from;
        if (params.check_in_to)
            where.check_in_date.lte = params.check_in_to;
    }
    if (params.check_out_from || params.check_out_to) {
        where.check_out_date = {};
        if (params.check_out_from)
            where.check_out_date.gte = params.check_out_from;
        if (params.check_out_to)
            where.check_out_date.lte = params.check_out_to;
    }
    if (params.min_amount || params.max_amount) {
        where.total_amount = {};
        if (params.min_amount)
            where.total_amount.gte = params.min_amount;
        if (params.max_amount)
            where.total_amount.lte = params.max_amount;
    }
    const orderBy = params.sort_by ? { [params.sort_by]: params.sort_order ?? 'asc' } : { created_at: 'desc' };
    ...
    return { bookings, total, page, limit, total_pages: Math.ceil(total / limit) };
}
...
// Overloaded function - handles both simple and role-based versions
export async function createBooking(dataOrPayload, userRole, userId) {
    // If role and userId are provided, use role-based version
    if (userRole && userId && 'check_in_date' in dataOrPayload) {
        const data = dataOrPayload;
        // Validate dates
    ...
    if (+payload.check_out_date <= +payload.check_in_date)
        throw BadRequest('check_out_date must be after check_in_date');
    ...
    if (payload.guests > room.capacity)
        throw BadRequest('Guests exceed room capacity');
...
// Overloaded function - handles both simple and role-based versions
export async function updateBooking(id, dataOrChanges, userRole, userId) {
    // If role and userId are provided, use role-based version
    if (userRole && userId && 'check_in_date' in (dataOrChanges || {})) {
        const data = dataOrChanges;
...
export async function checkOut(booking_id, data) {
    const booking = await prisma.booking.findUnique({ where: { id: booking_id }, select: { id: true, status: true } });
    if (!booking)
        throw NotFound('Booking not found');
    if (booking.status !== 'checked_in')
        throw BadRequest('Booking not eligible for check-out');
    await prisma.$transaction([
...
export async function stats(date_from, date_to) {
    const where = {};
...
export async function arrivals(date) {
...
export async function departures(date) {
    const target = date ?? new Date();
    const start = new Date(target);
    start.setHours(0, 0, 0, 0);
    const end = new Date(start);
