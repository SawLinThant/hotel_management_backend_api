...
export async function listUsers(params) {
    ...
    if (typeof params.email_verified === 'boolean')
        where.email_verified = params.email_verified;
    if (params.created_from || params.created_to) {
        where.created_at = {};
        if (params.created_from)
            where.created_at.gte = params.created_from;
        if (params.created_to)
            where.created_at.lte = params.created_to;
    }
    if (params.search) {
        where.OR = [
...
export async function bulkUpdate(user_ids, data) {
    await prisma.user.updateMany({ where: { id: { in: user_ids } }, data });
}
export async function bulkDelete(user_ids) {
...
export async function userStats() {
    ...
    return { total_users, admin_users, staff_users, guest_users, active_users, verified_users };
}
