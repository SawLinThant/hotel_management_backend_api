...
function signRefreshToken(userId: string, role: Role) {
...
export async function registerGuest(payload: {
	...
	return { user, access_token, refresh_token, token_type: 'Bearer', expires_in: 900 } as const;
}
...
export async function loginWithRole(payload: { email: string; password: string; role: Role }) {
	...
	const access_token = signAccessToken(user.id, user.role as Role);
	const refresh_token = signRefreshToken(user.id, user.role as Role);
	await prisma.refreshToken.create({
...
export async function refreshAccessToken(refreshToken: string) {
	...
	const access_token = signAccessToken(tokenRecord.user.id, tokenRecord.user.role as Role);
