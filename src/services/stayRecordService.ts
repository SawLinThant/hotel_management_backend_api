...
// Types for stay record operations
...
export async function createStayRecord(
	...
	const stayRecord = await prisma.stayRecord.create({
		data: {
			...
			additional_charges: undefined as unknown as never,
		},
		include: {
			booking: {
				include: {
...
export async function getStayRecordById(
	id: string,
	userRole: Role,
	userId: string
): Promise<any> {
...
export async function getStayRecords(
	filters: StayRecordFilters = {},
	pagination: PaginationOptions = {},
	userRole: Role,
	userId: string
): Promise<StayRecordListResponse> {
	const {
		booking_id,
		guest_id,
		room_id,
	...
	if (check_in_date_from || check_in_date_to) {
		...
		if (check_in_date_from) where.actual_check_in_time.gte = check_in_date_from;
		if (check_in_date_to) where.actual_check_in_time.lte = check_in_date_to;
	}

	if (check_out_date_from || check_out_date_to) {
		where.actual_check_out_time = {};
		if (check_out_date_from) where.actual_check_out_time.gte = check_out_date_from;
		if (check_out_date_to) where.actual_check_out_time.lte = check_out_date_to;
	}
	...
	if (userRole === 'guest') {
		...
		if (guest_id) where.booking = { guest_id };
		if (room_id) where.booking = { room_id };
	}
...
export async function deleteStayRecord(id: string, userRole: Role): Promise<{ message: string }> {
	if (userRole === 'guest') {
		throw new Error('Access denied');
	}
...
export async function getStayRecordStats(userRole: Role, userId: string): Promise<any> {
	const where: any = {};
